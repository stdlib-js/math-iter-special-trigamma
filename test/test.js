...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterTrigamma, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterTrigamma( uniform( 0.01, 50.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterTrigamma( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the trigamma function for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
	...
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = trigamma( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the trigamma function for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': trigamma( values[ 0 ] ),
			'done': false
		},
		{
			'value': trigamma( values[ 1 ] ),
			'done': false
		},
		{
			'value': trigamma( values[ 2 ] ),
			'done': false
		},
		{
			'value': trigamma( values[ 3 ] ),
			'done': true
		}
	];
